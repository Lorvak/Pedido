...
public class ActivityListarPedidos extends Activity {

	private static ActivityListarPedidos  tela;
	private static int[] idsRegistros=null;
	private static int idExcluir=-1;
	
	public ActivityListarPedidos   getTela(){
		return tela;
	}
	...
	public static void setIdExcluir(int idExcluir) {
		ActivityListarPedidos .idExcluir = idExcluir;
	}

	@Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_activity_listar_pedidos);
        this.atualizaRegistros(new Date());
        ListView listViewRegistros=(ListView)findViewById(R.id.listViewRegistros);
        ActivityListarPedidos .tela=this;
        listViewRegistros.setOnItemClickListener(new OnItemClickListener() {
            public void onItemClick(AdapterView<?> adapter, View view,      int pos, long id) {
               ActivityListarPedidos .setIdExcluir(ActivityListarPedidos .tela.getIdsRegistros()[pos]);
               AlertDialog.Builder builder = new AlertDialog.Builder(ActivityListarPedidos .tela);
           	   builder.setMessage("Você deseja excluir este registro?");
    		   builder.setTitle("Ponto Eletrônico");
    		   builder.setPositiveButton ("Sim", new DialogInterface.OnClickListener() {
    		      public void onClick(DialogInterface dialog, int which) {  
    		    	   	try{
    		           		   int value=tela.deletaRegistro(ActivityListarPedidos .getIdExcluir());
    		           		   if(value!=-1){
    		           			  AlertDialog.Builder builder = new AlertDialog.Builder(ActivityListarPedidos .tela);
    	    		    		  builder.setMessage("Registro excluído!");
    	    		    		  builder.setPositiveButton("OK", null);
    	    		    		  builder.show();
    	    		    		  ActivityListarPedidos .tela.btAtualizaRegistros(null);
    		           		   }
    		           	   }catch(Exception e){
